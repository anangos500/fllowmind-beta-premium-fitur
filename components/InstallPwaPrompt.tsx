...
const InstallPwaPrompt: React.FC<InstallPwaPromptProps> = ({ isVisible, isIos, onInstall, onClose }) => {
  ...
  return (
    <div
      className={`fixed bottom-24 left-4 right-4 sm:top-4 sm:right-4 sm:left-auto sm:bottom-auto sm:max-w-md z-50 transition-all duration-300 ease-out ${show ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12 sm:-translate-y-12'}`}
      role="alert"
      aria-live="polite"
    >
      <div className="bg-white/90 dark:bg-slate-800/90 backdrop-blur-md rounded-xl shadow-2xl p-4 flex items-center border border-slate-100 dark:border-slate-700">
        {isIos ? iosInstructions : desktopAndroidPrompt}
        <button
          onClick={handleClose}
          className="absolute -top-2 -right-2 p-1 bg-slate-200 dark:bg-slate-700 rounded-full text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600 shadow-sm"
          aria-label="Tutup"
        >
