...
import { Profile, ShopItem, FateCard } from '../types';
...
interface AuthContextValue {
  ...
  buyItem: (item: ShopItem) => Promise<{ success: boolean; message: string }>;
  equipItem: (type: 'head' | 'face' | 'skin' | 'theme', itemId: string | null) => Promise<void>;
  
  // Fate Card
  setDailyFateCard: (card: FateCard) => Promise<void>;
}
...
export const AuthProvider: React.FC<PropsWithChildren<{}>> = ({ children }) => {
  ...
  const addCoins = async (amount: number) => {
    ...
    // Apply Buffs from Active Fate Card
    if (profile.activeFateCard?.buffType === 'coin_multiplier' && amount > 0) {
        // Only multiply positive income, not deductions
        finalAmount = Math.floor(amount * profile.activeFateCard.buffValue);
    }

    const currentCoins = profile.coins || 0;
    // Prevent negative balance if deducting coins
    const newCoins = Math.max(0, currentCoins + finalAmount);
    await updateUserProfile({ coins: newCoins });
  };

  const buyItem = async (item: ShopItem): Promise<{ success: boolean; message: string }> => {
    if (!user || !profile) return { success: false, message: 'User not found' };
    
    const currentCoins = profile.coins || 0;
    const inventory = profile.inventory || [];

    if (inventory.includes(item.id)) {
        return { success: false, message: 'Item sudah dimiliki!' };
    }

    // Apply Discount Buff if Active
    let finalPrice = item.price;
    if (profile.activeFateCard?.buffType === 'shop_discount') {
        finalPrice = Math.floor(item.price * (1 - profile.activeFateCard.buffValue));
    }
    ...
    try {
        await updateUserProfile({ coins: newCoins, inventory: newInventory });
        return { success: true, message: 'Pembelian berhasil!' };
    } catch (e) {
        return { success: false, message: 'Gagal memproses pembelian.' };
    }
  };

  const equipItem = async (type: 'head' | 'face' | 'skin' | 'theme', itemId: string | null) => {
      if (!user || !profile) return;
      const currentEquipped = profile.equipped || { head: null, face: null, skin: 'default', theme: 'default' };
      const newEquipped = { ...currentEquipped, [type]: itemId };
      await updateUserProfile({ equipped: newEquipped });
  };
  
  const setDailyFateCard = async (card: FateCard) => {
      if (!user || !profile) return;
      const today = new Date().toLocaleDateString('en-CA'); // YYYY-MM-DD
      
      // We don't convert FateCard to snake_case entirely because it is stored as a JSONB blob 'active_fate_card'
      // But updateUserProfile handles top-level keys.
