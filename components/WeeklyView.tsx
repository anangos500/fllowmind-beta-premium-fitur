...
const WeeklyView: React.FC<WeeklyViewProps> = ({ tasks, onSelectTask, onUpdateTask, onDeleteTask }) => {
  ...
  const handleDeleteRequest = (e: React.MouseEvent, taskId: string) => {
    e.stopPropagation();
    // FIX: Hapus pembatasan penghapusan proyeksi. useTasks hook sekarang menanganinya.
        ...
        {weekDays.map(day => {
          ...
          return (
            <div 
              ...
              className={`rounded-xl p-4 flex flex-col transition-all duration-200 ${isToday ? 'bg-blue-50 dark:bg-slate-800 border-2 border-blue-600 dark:border-blue-600 dark:shadow-[0_0_15px_rgba(37,99,235,0.15)]' : 'bg-white dark:bg-slate-800 shadow-sm'} ${isSelected ? 'ring-2 ring-blue-600 shadow-lg' : 'lg:hover:shadow-md lg:cursor-pointer'}`}
            >
              <div className={`text-center mb-4 ${isToday ? 'font-bold text-blue-600 dark:text-blue-400' : 'text-slate-800 dark:text-slate-200'}`}>
                ...
                {dayTasks.length > 0 ? dayTasks.map(task => {
                  ...
                  return (
                    <div 
                      ...
                      className={`p-2 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700 transition-all duration-300 group flex items-center justify-between ${wasJustMoved ? 'ring-2 ring-green-500 bg-green-50 dark:bg-green-900/50' : 'bg-slate-50 dark:bg-slate-700/50'} ${isProjected ? 'opacity-60 cursor-default' : 'cursor-pointer'}`}
                    >
                      ...
                      <div className="flex items-center ml-1 flex-shrink-0">
                          ...
                          {/* Tombol hapus sekarang tersedia untuk semua tugas termasuk proyeksi */}
                          <button
