...
const TaskDetailModal: React.FC<TaskDetailModalProps> = ({ task, tasks, onUpdate, onDelete, onStartFocus, onClose }) => {
  ...
  useEffect(() => {
    const hasChanged = JSON.stringify(editedTask) !== JSON.stringify(task);
  ...
  // Add helper to detect overlay click specifically
  ...
  const handleChecklistChange = (itemId: string, completed: boolean) => {
    ...
    const newStatus = allCompleted 
      ? TaskStatus.Done 
      : editedTask.status === TaskStatus.Done ? TaskStatus.ToDo : editedTask.status;

    setEditedTask(prev => ({ ...prev, checklist: updatedChecklist, status: newStatus }));
  };
  ...
  const handleAiChecklist = async () => {
    ...
    try {
      // FIX: Changed from /.netlify/functions/gemini to /api/gemini for Cloudflare support
      const response = await fetch('/api/gemini', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          model: 'gemini-2.5-flash',
          contents: `Berdasarkan judul tugas "${editedTask.title}", buat daftar detail tugas atau langkah-langkah yang dapat ditindaklanjuti. Hasilkan semua item daftar HANYA dalam Bahasa Indonesia.`,
          config: {
  ...
  const handleSave = async () => {
    ...
    if (conflictingTask) {
        setError(`Waktu tugas ini tumpang tindih dengan tugas: "${conflictingTask.title}".`);
        return;
    }
  ...
  return (
    <div onClick={handleOverlayClick} className={`fixed inset-0 bg-black flex justify-center items-center z-50 p-4 transition-opacity duration-300 ${isAnimatingOut ? 'bg-opacity-0' : 'bg-opacity-50'}`}>
      <div onClick={(e) => e.stopPropagation()} className={`bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col transition-all duration-300 ease-out overflow-hidden ${isAnimatingOut ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
        <header className="p-5 sm:p-6 border-b dark:border-slate-700 flex justify-between items-center">
          ...
          <button onClick={handleCloseRequest} className="text-slate-500 hover:text-slate-800 dark:text-slate-300 dark:hover:text-slate-200"><XIcon className="w-6 h-6"/></button>
        </header>
              ...
              {editedTask.checklist.map(item => (
                <div key={item.id} className="flex items-center group">
                  ...
                  <button
                    ...
                    aria-label="Hapus item detail task"
                  >
        ...
        <footer className="p-5 sm:p-6 bg-slate-50 dark:bg-slate-800/50 border-t dark:border-slate-700 flex flex-wrap gap-4 justify-between items-center">
          <div className="flex items-center gap-4">
              ...
              <button 
                  ...
                  className="text-sm font-semibold text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 flex items-center disabled:text-slate-400"
                  disabled={isSaving}
                  title="Tambahkan ke Kalender Eksternal"
              >
                  <CalendarCheckIcon className="w-5 h-5 mr-1.5" />
                  <span className="hidden sm:inline">Ke Kalender</span>
              </button>
          </div>
