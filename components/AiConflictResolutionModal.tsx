...
const AiConflictResolutionModal: React.FC<AiConflictResolutionModalProps> = ({ taskTitle, suggestedSlots, onClose, onSelectSlot, onManualAdd, conflictReason, conflictType }) => {
  ...
  // Tentukan judul dan ikon berdasarkan jenis konflik
  const modalTitle = conflictType === 'overlap' ? 'Jadwal Bentrok' : 'Waktu Sudah Berlalu';
    ...
    <div className={`fixed inset-0 bg-black flex justify-center items-center z-50 p-4 transition-opacity duration-300 ${isAnimatingOut ? 'bg-opacity-0' : 'bg-opacity-50'}`}>
      <div className={`bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md transition-all duration-300 ease-out overflow-hidden ${isAnimatingOut ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
        <header className="p-5 border-b dark:border-slate-700 flex justify-between items-center">
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">{modalTitle}</h2>
        ...
        <main className="p-6">
          <p className="text-slate-600 dark:text-slate-300 mb-4">
            Jadwal untuk <span className="font-bold text-slate-800 dark:text-slate-100">"{taskTitle}"</span> tidak dapat dibuat karena {conflictReason}.
          </p>
          <p className="text-sm font-semibold text-slate-500 dark:text-slate-400 mb-3">
            Saran waktu pengganti:
          </p>
